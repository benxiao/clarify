...
A simple Interactive power graph

interaction: the grey nodes can be expanded and the expanded nodes can be shrinked


Example. click on a node that has two child nodes
Assume B has two child nodes {B1, B2}, and connects to C, and is connected by A

{source:A, target:B}, {source:B, target:C} are replaced with
    {source:A , target:B1}
    {source:A.  target:B2}
    {source:B1, target:C}
    {source:B2, target:C}

Unfortunately, it is difficult to use the same technique to show nested group structures. something can be added
in the future. we may have to generate our own power graph structures, the current understanding is that the
powerGraphGroups sets the states of the forcelayout internally, so mutate the generated power graph will not work, as
it will no longer match the forcelayout "internal representation" and will cause it to break

the other thing is that the power graph generates its own "links" and "groups" through the call powerGraphGroups, the
specified links may not always be interepreted the right way. For examples, it is difficult to show three nodes going to
a common fourth nodes without the powerGraphGroups collapsing them in a group with a single arrow going to the fourth
node
 */
...
function initial_layout() {
    ...
    d3.json(jsonfile, function (error, graph) {
        ...
        // find the suitable width for all nodes
        let allNodes = [];
